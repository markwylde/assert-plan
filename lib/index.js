...
function plannedAssert (expectedAssertions, options) {
  ...
  const createAssert = (name) => {
    return (...args) => {
      typeof name === 'function' ? name(...args) : assert[name](...args);
      passes = passes + 1;

      if (resolved) {
        throw new Error(`expected assertions to equal ${expectedAssertions}, but had ${passes}`);
      }
  ...
  return {
    ...
    ok: createAssert('ok'),
    notOk: createAssert((value, message) => {
      ...
      const waitItem = {
        ...
        fn: () => {
          if (passes !== expectedAssertions) {
            error.message = `expected assertions to equal ${expectedAssertions}, but counted ${passes}`;
            throw error;
          }
        },
        callback
